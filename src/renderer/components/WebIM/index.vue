		...
		<div class="WebIM" v-for="(item,index) in webIMList" :key="index" :style="{width:item.width}">
			...
			<div class="winMin" v-if="item.isShow">
				...
				<div class="conWrap" :style="{height:item.height}">
					...
					<Transfer v-if="transferIdx==index" :user-id="item.userId" :user-name="item.userName" v-on:changeStatus="__exitTransfer"></Transfer>
				</div>
				<div class="input">
					<input type="text" placeholder="输入讯息" @keyup.enter="__submitMsg(index,item.userId)">
					<div>
						<span class="icon"></span>
						<span class="icon">
							<input type="file" @change="__chooseImg(index)" id="fileId">
						</span>
						<span class="icon"></span>
...
export default {
    ...
    components: { ChangeName, Transfer },
    watch: {
        webIMList: {
    ...
    mounted() {
        ...
        let res = createSeed(18625910960, "453512494");
        console.log(res);
    },
    methods: {
        ...mapMutations([
        ...
        __exitTransfer(val) {
            this.transferIdx = 99;
            console.log(this.transferIdx);
        },
        //发送文字消息
        async __submitMsg(index, userId) {
            var msg = {
                type: 0,
                messageType: 1,
