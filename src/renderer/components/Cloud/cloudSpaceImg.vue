<template>
    <div @click.stop="__judge" class="box">
        <div class="fileList">
            <div v-for="(item,index) in fileList" :key="index" class="fileList-item" @contextmenu="__showOperation(index)">
                <div @click="__previewImg(item)" class="img"></div>
                <div v-if="!item.isInputName" @click="__selectUpdateName(item)" style="text-align:center;width:100%;">{{item.fileName}}</div>
