            ...
            <div v-if="deal">
                <div v-if="!flag"
                    ...
                    <div v-if="item.nickname">
                        <strong>{{item.nickname}}</strong>{{$t('main.sentyouafriendrequest')}}
                    </div>
                    <div class="reqBtn">
                        <span class="add cp"
                              @click="__add">{{$t('main.accept')}}</span>
                        <span class='ignore cp'
                              @click="__ignore">{{$t('main.ignore')}}</span>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async __add() {
            ...
            if (res.msg === 'success') {
                this.flag = true
                this.text = this.$t('main.accepted')
            }
        },
        async __ignore() {
            let data = {
            ...
            if (res.msg === 'success') {
                this.flag = true
                this.text = this.$t('main.requestdeleted')
            }
        }
    },
    watch: {
