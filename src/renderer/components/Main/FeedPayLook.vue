<template>
    <div class="mask" v-if="payLookId">
        <div class="content" v-if="!sureShow">
            <div class="title">
                <p>{{$t('main.paytowatch')}}</p>
                <span class="x" @click="__out"></span>
            </div>
            ...
            <div class="rank">
                <div class="clearfix" v-for="(item,index) in priceList" :key="index" :class="{'selected':selected==index}" @click='__select(index,item)'>
                    <div class="fl"> {{$t('main.opt')}}</div>
                    <div class="fr">{{item.currency}}</div>
                    <div class="fr">{{item.price}}</div>
                </div>
            </div>

            <div class="btn" @click="__configReward">{{$t('confirm')}}</div>
        </div>
        ...
        <div class="content" v-if="sureShow">
            ...
            <div class="btn" @click="__out">{{$t('confirm')}}</div>
        </div>
    </div>
</template>
    ...
    methods: {
        ...
        async __getWorkInfo() {
            const res = await this.api.snsWorkInfo({
                userId: this.payLookItem.userId
            });
        ...
        async __configReward() {
            ...
            let params = {
                authorId: this.payLookItem.userId,
